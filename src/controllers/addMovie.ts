export async function importMoviesController(req, res) {
  try {
    const { db } = req.app;

    // Sample JSON data (this should normally be loaded from a file)
    const moviesData = [
      {
        "Title": "Avatar",
        "Year": "2009",
    ...
    const result = await db.collection('movies').insertMany(moviesData);

    if (result.acknowledged) {
      res.status(200).json({ message: 'Movies imported successfully', count: result.insertedCount });
    } else {
      throw new Error('Movies not imported');
    }
  } catch (error) {
